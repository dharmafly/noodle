...
function getDocument (shouldCache, options, deferred) {
  request(options, function (err, response, body) {
    if (err || response.statusCode !== 200) {
      ...
      if (shouldCache && pageCache.check(options.url)) {
        pageCache.put(options.url, {body: body, headers: response.headers});
        exports.events.emit('cache/page', pageCache.get(options.url));
      }
