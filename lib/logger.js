...
module.exports = function (events, config) {
  ...
  events.on('cache/results', function (cacheResult) {
    toTerminal('➡ Cache: Result has been cached');
  });

  // Called when a result was retrieved from the cache
  events.on('cache/results/retrieved', function (cacheResult) {
    toTerminal('⬅ Cache: Result was retrieved from cache');
  });
