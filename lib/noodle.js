  ...
  request(options, function (err, response, body) {
    if (err || response.statusCode !== 200) {
      ...
      if (shouldCache && !pageCache.check(options.uri)) {
        //added response.request in order to get the details like location and domain
        pageCache.put(options.uri, {body: body, headers: response.headers, request: response.request});
...
exports._wrapResults = function (results, query) {
  ...
  if (results.length || Object.keys(results).length) {
    ...
    if (query.request) {
      resultSet.request = getRequestDetailsForResultSet(query);
    }
...
// Function called from exports._wrapResults()
//
// Passed in a query and returns the full request headers
// or specific request headers as specified by the query.
function getRequestDetailsForResultSet(query) {
  var bucket       = {},
    requestHeaders = pageCache.get(query.url).value.request,
    prop;

  if (query.request !== 'all' && _.isArray(query.request)) {
    for (prop in requestHeaders) {
      query.request.forEach(function (name) {
        if(prop.toLowerCase() === name.toLowerCase()) {
          bucket[name] = requestHeaders[prop];
        }
      });
    }
    return bucket;
